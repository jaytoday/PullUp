...
function defectProbability(p: CostParams, type: ChangeType): number {
  return p.baseDefectProbability[type] ?? p.defaultDefectProbability;
}

/** Expected defect cost if approved at wait `w`, in dev-hours. */
export function expectedDefectCost(w: number, p: CostParams, type: ChangeType): number {
  const pd = defectProbability(p, type);
...
export function marginalReviewBenefit(w: number, p: CostParams, type: ChangeType): number {
  const pd = defectProbability(p, type);
  const cd = defectCost(p, type);
  const t = w / p.reviewWindowHours;
...
export function totalCost(w: number, p: CostParams, type: ChangeType): number {
  return delayCost(w, p) + expectedDefectCost(w, p, type);
}
 ...
 */
export function findOptimalWait(p: CostParams, type: ChangeType): number {
  const step = 1;
  let best = p.minWaitHours;
  let bestCost = totalCost(best, p, type);
  for (let w = p.minWaitHours + step; w <= p.maxWaitHoursCap; w += step) {
    const c = totalCost(w, p, type);
    if (c < bestCost) {
      bestCost = c;
      best = w;
    }
