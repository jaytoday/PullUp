...
export const Command: Component<CommandProps> = (props) => {
  ...
  const ctx: CommandContextValue = {
    ...
    setSearch: (val) => {
      setSearch(val);
      setActiveIndex(0);
      // PullUp: highlight the first match so Enter's target is visible.
      queueMicrotask(() => updateActiveAttribute(0));
    },
    activeIndex,
    ...
    <Dialog open={isOpen()} onOpenChange={setOpen}>
      <Dialog.Portal>
        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
          <Dialog.Content
            // PullUp: focus the search input on open (the container is tabbable and would otherwise win).
